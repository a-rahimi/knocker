...
static void IRAM_ATTR gpio_isr_handler(void *arg) {
  const xQueueHandle queue = (xQueueHandle)arg;

  TickType_t t = xTaskGetTickCountFromISR();

  // Only recognize this as an event if there was plenty of delay since the last
  // event.
  if (t - last_isr_tick > 200 / portTICK_PERIOD_MS) {
    Event event = EV_PUSH;
...
static void microphone_task(void *arg) {
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      // Wait for a button.
      if (xQueueReceive(microphone_event_queue, &event,
                        800 / portTICK_PERIOD_MS)) {
        printf("Button push!\n");
        ...
        if (num_timeouts++ > 3) {
          // This is the end of the code sequence. Reset and wait for a new
          // sequence.
          break;
        }
...
void app_main(void) {
  gpio_config((gpio_config_t[]){{
      .pin_bit_mask = (1ULL << GPIO_OUTPUT_IO_0) | (1ULL << GPIO_OUTPUT_IO_1),
  ...
  gpio_config((gpio_config_t[]){{.pin_bit_mask = 1ULL << GPIO_INPUT_BUTTON,
                                 .mode = GPIO_MODE_INPUT,
                                 .intr_type = GPIO_INTR_NEGEDGE,
                                 .pull_up_en = 1,
                                 .pull_down_en = 0}});

  gpio_config((gpio_config_t[]){{.pin_bit_mask = 1ULL << GPIO_INPUT_MIC,
                                 .mode = GPIO_MODE_INPUT,
                                 .intr_type = GPIO_INTR_NEGEDGE,
                                 .pull_up_en = 1,
                                 .pull_down_en = 0}});
  ...
  gpio_isr_handler_add(GPIO_INPUT_MIC, gpio_isr_handler,
                       (void *)microphone_event_queue);
  gpio_isr_handler_add(GPIO_INPUT_BUTTON, gpio_isr_handler,
                       (void *)button_event_queue);
